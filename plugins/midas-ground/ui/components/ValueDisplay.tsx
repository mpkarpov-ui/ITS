// Ported from GSS's reusable/ValueDisplay.jsx.
...
export function MultiValue({
  ...
  // Default to theme tokens; callers override per-cell via labelColors/dataColors.
  const lc = labelColors ?? titles.map(() => 'var(--text-dim)');
  const dc = dataColors ?? titles.map(() => 'var(--text)');
